...
const COPY = {
  ready: { title: "Ready for Pickup", sub: "Orders from assigned farmers waiting for a driver.", filter: "ready", empty: "No ready-for-pickup orders yet." },
  assigned: { title: "Assigned Pickups", sub: "Pickups with a driver assigned.", filter: "assigned", empty: "No assigned pickups." },
  requests: { title: "Ready for Pickup", sub: "Orders from assigned farmers waiting for a driver.", filter: "ready", empty: "No ready-for-pickup orders yet." },
  today: { title: "Today's Pickups", sub: "Scheduled for today and not yet picked up.", filter: "today", empty: "No pickups scheduled today." },
  active: { title: "All Pickups", sub: "Every pickup for assigned farmers, grouped by lot / batch.", filter: "all", empty: "No pickups yet." },
  all: { title: "All Pickups", sub: "Every pickup for assigned farmers, grouped by lot / batch.", filter: "all", empty: "No pickups yet." },
  ...
  history: { title: "Picked Up", sub: "Completed pickup history for farmers.", filter: "history", empty: "No pickup history yet." },
};
...
function pickupPath(pickup, isIncoming) {
  ...
  return receive ? `/vendor/pickups/${pickup.id}/receive` : `/vendor/pickups/${pickup.id}`;
}
...
function orderPath(pickup) {
  ...
  return `/vendor/orders/detail/${encodeURIComponent(id)}?${params.toString()}`;
}
    ...
    <div className="space-y-4 p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-wrap items-end gap-2">
        ...
        <div className="flex items-center gap-1.5 pt-4 sm:pt-0">
          <button
            type="button"
            className={`${EXCEL_BTN} !min-h-9 !py-1.5 !text-xs`}
            onClick={() => setPickupDate(todayISODate())}
          >
            Today
          </button>
          <button
            type="button"
            className={`${EXCEL_BTN} !min-h-9 !py-1.5 !text-xs`}
            onClick={() => setPickupDate(yesterdayISODate())}
          >
            Yesterday
          </button>
        </div>
        {hasFilter ? (
          <button
            type="button"
            className={`${EXCEL_BTN} !min-h-9 !py-1.5 !text-xs text-rose-700`}
            onClick={() => {
              setQ("");
              setFarmerId("");
              setProduct("");
