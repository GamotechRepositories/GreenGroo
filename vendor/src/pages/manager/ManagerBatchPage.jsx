...
export default function ManagerBatchPage() {
  ...
  if (!data && !error) return <p className="p-4 sm:p-6 lg:p-8 text-xs text-[#6B7280]">Loading batch…</p>;
  if (!data) return <p className="p-4 sm:p-6 lg:p-8 text-xs text-red-600">{error}</p>;
  ...
  return (
    <div className="space-y-4 p-4 sm:p-6 lg:p-8">
        ...
        to={
          location.state?.from === "all"
            ...
            : location.state?.from === "centre"
              ? "/vendor/pickups/centre"
              : "/vendor/pickups/incoming"
        }
      ...
      <div className="grid grid-cols-2 gap-2.5 rounded-2xl border border-slate-200/80 bg-white p-3 shadow-sm sm:gap-3 sm:p-4 sm:grid-cols-3">
        ...
        <Fact label="Vehicle Number" value={data.vehicleNumber} />
        <Fact label="Farmers" value={data.farmers?.join(", ") || "—"} />
        <Fact label="Products" value={data.products?.join(", ") || "—"} />
        <Fact label="Status" value={pickupStatusLabel(data.status || data.liveStatus) || "On the way to centre"} />
      </div>

      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 pb-2">
        <h2 className="text-sm font-bold text-[#1F2937]">Orders in this batch</h2>
        <span className="text-xs text-[#6B7280]">Receive order by order or scan QR</span>
      </div>

      {orders.length === 0 ? (
        <p className="text-xs text-[#6B7280]">No orders found for this batch.</p>
      ) : (
        <>
