...
export default function ManagerReceivePage() {
  ...
  const save = async (status) => {
    ...
    try {
      ...
      if ((status || form.receivingStatus) === "RECEIVED") {
        ...
        if (qualityId) {
          navigate(`/vendor/quality/${encodeURIComponent(qualityId)}`);
        } else {
          navigate("/vendor/quality/inspection");
        }
      } else {
  ...
  if (!pickup && !error) return <p className="p-4 sm:p-6 lg:p-8 text-xs text-[#6B7280]">Loading incoming pickup…</p>;
      ...
      {step === "scan" ? (
        <section className={EXCEL_PANEL}>
          ...
          <div className="space-y-2 p-3">
            ...
            <Link to="/vendor/pickups/incoming" className={EXCEL_BTN}>Back</Link>
          </div>
        </section>
      ) : null}
      ...
      {step === "photos" ? (
        <>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {done ? (
              <Link to={`/vendor/quality/${encodeURIComponent(pickup.orderId || pickup.orderDisplayId || "")}`} className={EXCEL_BTN}>
