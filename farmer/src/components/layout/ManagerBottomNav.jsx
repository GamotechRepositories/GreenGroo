...
const TABS = [
  { to: "/farmer/manager/dashboard", label: "Home", icon: HomeIcon, end: true },
  { to: "/farmer/manager/search", label: "Search", icon: SearchIcon },
  { to: "/farmer/manager/quality/pending", label: "Alerts", icon: BellIcon },
  { to: "/farmer/manager/pickups/all", label: "History", icon: HistoryIcon },
];
...
export default function ManagerBottomNav() {
  ...
  const openScanned = async (value) => {
    ...
    if (match) {
      ...
      navigate(
        isAtCentre(match)
          ? `/farmer/manager/pickups/${match.id}/receive`
          : `/farmer/manager/pickups/${match.id}`
      );
      return;
    }
