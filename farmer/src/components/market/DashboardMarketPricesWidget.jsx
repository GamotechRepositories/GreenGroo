...
export default function DashboardMarketPricesWidget() {
  const tickerCrops = CROP_MARKET_PRICES.slice(0, 5);
  const tableCrops = CROP_MARKET_PRICES.slice(0, 4);
  const hasPrices = CROP_MARKET_PRICES.length > 0;
  ...
  return (
    <section className={EXCEL_PANEL}>
      ...
      <div className="p-3 space-y-3">
        {!hasPrices ? (
          <p className="py-8 text-center text-xs text-slate-500">No mandi price data available.</p>
        ) : (
          <>
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5">
              {tickerCrops.map((crop) => {
                ...
                <tbody>
                  {tableCrops.map((crop) => (
                    <tr key={crop.id} className="hover:bg-[#F2F2F2]">
                      ...
                      <td className={`${EXCEL_CELL} text-[#4B5563]`}>{crop.primaryMandi || "—"}</td>
