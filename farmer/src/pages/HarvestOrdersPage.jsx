  ...
  const spreadsheetRows = useMemo(() => {
    ...
    orders.forEach((o) => {
      ...
      if (searchQuery.trim()) {
        ...
        const hay = `${orderId} ${prodName} ${varietyName}`.toLowerCase();
    ...
    return rows;
  }, [orders, productFilter, searchQuery, availableGrades]);
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-[#6B7280]">
            Daily harvest records and produce grade summary • {spreadsheetRows.length} record{spreadsheetRows.length === 1 ? "" : "s"}
          </p>
        </div>
        ...
        <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between flex-wrap">
          ...
          <div className="flex items-center gap-2 min-w-0 flex-1">
            ...
            <select
              ...
              className="w-full sm:w-auto min-w-[180px] max-w-xs rounded-lg border border-[#D1D5DB] bg-white px-3 py-1.5 text-xs font-semibold text-[#1F2937] shadow-xs focus:border-emerald-600 focus:outline-none focus:ring-1 focus:ring-emerald-600"
            >
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description={
            productFilter !== "ALL" || searchQuery
              ? "No harvest orders match the selected filter or search criteria."
          ...
          <div className="space-y-2.5 md:hidden">
            {spreadsheetRows.map((row, idx) => (
              <HarvestMobileCard
                ...
                onOpen={() => row.orderId && navigate(`/farmer/orders/${row.orderId}`)}
              />
            ))}
          </div>
            ...
            <table className="w-full table-fixed border-collapse text-[10px] sm:text-[11px]">
              ...
              <tbody>
                {spreadsheetRows.map((row, idx) => {
                  ...
                  return (
                    <tr
                      key={`${row.orderId}-${idx}`}
                      onClick={() => row.orderId && navigate(`/farmer/orders/${row.orderId}`)}
                      className="hover:bg-[#F2F8F3] transition-colors cursor-pointer group"
                      title="Click row to view full order details"
                    >
                      ...
                      <td className={`${TD} truncate`} title={row.productName}>
                        <p className="font-bold text-[#1F2937] truncate group-hover:text-emerald-800">{row.productName || "Product"}</p>
                      </td>
                      {/* Variety Column */}
              ...
              <tfoot>
                <tr className="bg-[#EBF5EB] font-bold text-[#1F2937] border-t-2 border-[#217346]">
                  <td colSpan={7} className="border border-[#C5D4C8] px-2 py-2 text-right uppercase tracking-wider text-[10px] sm:text-[11px] text-[#217346]">
                    Grand Total:
                  </td>
                  {availableGrades.map((g) => (
...
function HarvestMobileCard({ row, availableGrades, onOpen }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex min-w-0 items-center justify-between gap-1.5 border-b border-slate-100 pb-2">
