...
export default function ManagerOrderDetailPage() {
  ...
  if (!order) {
    return (
      <EmptyState
        ...
        action={
          <Link to="/manager/orders" className={EXCEL_BTN_PRIMARY}>
            Back to Orders
          </Link>
        }
      />
  ...
  const backTo = fromPickups ? "/manager/pickups/ready" : "/manager/orders";
  ...
  const editPath = `/manager/orders/create?${formParams.toString()}`;
