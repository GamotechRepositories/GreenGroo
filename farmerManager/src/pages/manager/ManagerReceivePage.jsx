  ...
  const save = async (status) => {
    ...
    try {
      ...
      if ((status || form.receivingStatus) === "RECEIVED") {
        ...
        if (qualityId) {
          navigate(`/manager/quality/${encodeURIComponent(qualityId)}`);
        } else {
          navigate("/manager/quality/inspection");
        }
      } else {
      ...
      {step === "scan" ? (
        <section className={EXCEL_PANEL}>
          ...
          <div className="space-y-2 p-3">
            ...
            <Link to="/manager/pickups/incoming" className={EXCEL_BTN}>Back</Link>
          </div>
        </section>
      ) : null}
      ...
      {step === "photos" ? (
        <>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {done ? (
              <Link to={`/manager/quality/${encodeURIComponent(pickup.orderId || pickup.orderDisplayId || "")}`} className={EXCEL_BTN}>
            ...
            <Link to="/manager/pickups/incoming" className={EXCEL_BTN}>Incoming Pickups</Link>
          </div>
        </>
      ) : null}
