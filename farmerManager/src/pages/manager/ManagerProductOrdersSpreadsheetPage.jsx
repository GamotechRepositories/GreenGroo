...
export default function ManagerProductOrdersSpreadsheetPage() {
  ...
  const createPath = `/manager/orders/create${productId ? `?productId=${encodeURIComponent(productId)}` : ""}`;

  return (
    <div className="space-y-3 text-xs">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="min-w-0">
          <Link to="/manager/orders" className="text-[11px] font-semibold text-[#217346] hover:underline">
            ← Orders
          </Link>
              ...
              {spreadsheetRows.map((row, idx) => (
                <tr key={`${row.orderId}-${row.farmerId}-${idx}`} className="border-b border-[#D4D4D4] last:border-0 hover:bg-[#F9F9F9]">
                  ...
                  <td className="px-2 py-1.5 font-semibold">
                    <Link to={`/manager/orders/farmer/${row.farmerId}`} className="text-[#217346] hover:underline">
                      {row.farmerName}
                    </Link>
                  </td>
