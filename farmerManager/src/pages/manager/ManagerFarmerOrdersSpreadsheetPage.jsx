...
export default function ManagerFarmerOrdersSpreadsheetPage() {
  ...
  if (!farmer) {
    return (
      <div className={`${EXCEL_PANEL} p-8 text-center text-xs text-[#DC2626] space-y-3`}>
        ...
        <Link to="/manager/orders" className={`${EXCEL_BTN_PRIMARY} inline-block px-3 py-1.5`}>
  ...
  return (
    <div className="space-y-4 font-sans text-xs">
      ...
      {/* ──────────────────────────────────────────────────────────── */}
      {/* HARVEST ORDER DETAILS SPREADSHEET (PHOTO REPLICA)            */}
      {/* ──────────────────────────────────────────────────────────── */}
      <section className={EXCEL_PANEL}>
        {/* Panel Top Header - Beautifully Aligned */}
        <div className={`${EXCEL_PANEL_HEAD} flex flex-wrap items-center justify-between gap-3 bg-[#F2F8F3] px-3 py-2 border-b border-[#D4D4D4]`}>
          {/* Left Group: Badges & Filter Produce Chips */}
          <div className="flex items-center gap-2 flex-wrap min-w-0">
            <span className="rounded bg-[#217346] px-2 py-0.5 text-[10px] font-bold text-white whitespace-nowrap">
              FARMER: {farmer.name.toUpperCase()}
            </span>
