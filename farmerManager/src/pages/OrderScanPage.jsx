...
export default function OrderScanPage() {
  ...
  if (!order) {
    return (
      <EmptyState
        ...
        action={
          <Link to={isManager ? "/manager/orders" : "/farmer/orders/new"} className={EXCEL_BTN_PRIMARY}>
            Back to Orders
          </Link>
        }
      />
  ...
  const detailPath = isManager
    ? `/manager/orders/detail/${encodeURIComponent(orderId)}${order.farmerId ? `?farmerId=${encodeURIComponent(order.farmerId)}` : ""}`
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-3">
      ...
      <div className="flex flex-col-reverse gap-2 pt-1 sm:flex-row sm:justify-end">
        <Link to={isManager ? "/manager/orders" : "/farmer/orders/new"} className={`${EXCEL_BTN} !min-h-10 w-full sm:w-auto`}>
          Back
        </Link>
